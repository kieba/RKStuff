...
public class BlockDistributionFluid extends BlockRK implements ITileEntityProvider, IPeripheralProvider {

    private IIcon[] icons = new IIcon[3];

    public BlockDistributionFluid() {
        super(Material.iron, Reference.BLOCK_DISTRIBUTION_FLUID);
    }

    @Override
    public TileEntity createNewTileEntity(World world, int meta) {
        return new TileDistributionFluid();
    }

    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        if (!super.onBlockActivated(world, x, y, z, player, side, hitX, hitY, hitZ)) {
            if (world.getTileEntity(x, y, z) instanceof TileDistributionFluid && ServerHelper.isServerWorld(world)) {
                player.openGui(RkStuff.INSTANCE, Reference.GUI_ID_DISTRIBUTION_FLUID, world, x, y, z);
            }
        }
        return true;
    }
    ...
    protected IIcon loadIconById(String suffix, IIconRegister iconRegister) {
        return iconRegister.registerIcon(Reference.MOD_ID + ":distribution/" + Reference.BLOCK_DISTRIBUTION_FLUID + suffix);
    }
