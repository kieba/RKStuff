...
public class GuiBoiler extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float tick, int mouseX, int mouseY) {
        ...
        int coolCoolantPx = Math.round(MAX * 0.5f);
        int hotCoolantPx = Math.round(MAX * 0.5f);
        int waterPx = Math.round(MAX * 0.5f);
        int steamPx = Math.round(MAX * 0.5f);
        int heatPx = Math.round(MAX * 0.5f);
        ...
        drawTexturedModalRect(x + 82, y + 71 - heatPx, 240, MAX - heatPx, 12, waterPx);
        ...
        if(isInArea(mouseX, mouseY, x + 21, yMin, x + 37, yMax)) {
            ...
            list.add(String.format("%d/%d mB", 1, 1000));
            ...
            List<String> list = new ArrayList<>(2);
            list.add("Heat:");
            list.add(String.format("%d °C", 1, 1000));
            this.func_146283_a(list, mouseX, mouseY);
